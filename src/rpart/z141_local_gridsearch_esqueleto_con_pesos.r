      ...
      for (vmin_split in c(800)) {
        for (vmin_bucket in c(160)) {
          ...
          for (vpeso1 in c(1,1.1,1.2,1.3,1.4,2,4,8,16,32,40)) {
            ...
            # if (ganancia_promedio < 50000000) {
            #    next
            # } 
            
            tb_grid_search <- rbindlist( 
              list( tb_grid_search, 
                    list( vmax_depth, vmin_split, vmin_bucket, vmin_cp, vpeso1, vpeso2, ganancia_promedio) ) )

          } # cierre del ciclo peso2
         } # cierre del ciclo peso1
          # agrego a la tabla
          
          # escribo la tabla a disco 
          Sys.sleep(1)  # espero un par de segundos
          
          fwrite( tb_grid_search,
                  file = archivo_salida,
                  sep = "\t" )# agrego a la tabla
